...
/*
	modified to work with nodered2mcu
*/
...
//	if (this.hdrout === true) { this.hdrout = "all"; }
	let tmpwarn = true;
	let node = this;
	let re = new RegExp(n.sep.replace(/[-[\]{}()*+!<=:?.\/\\^$|#\s,]/g,'\\$&') + '(?=(?:(?:[^"]*"){2})*[^"]*$)','g');
	re.sep = node.sep;
	...
	this.quo = n.quo;
	this.ret = n.ret;
	this.winflag = n.winflag;
	this.lineend = n.lineend;
	this.multi = n.multi;
	this.hdrin = n.hdrin;
	this.hdrout = n.hdrout;
	this.skip = n.skip;
	this.parsestrings = n.parsestrings;
	this.include_empty_strings = n.include_empty_strings;
	this.include_null_values = n.include_null_values;
	this.parsestrings = n.parsestrings;
	this.hdrout = n.hdrout;

	// pass in an array of column names to be trimmed, de-quoted and retrimmed
	let clean = function(col,sep) {
		if (sep && (sep !== re.sep)) {
			re = new RegExp(sep.replace(/[-[\]{}()*+!<=:?.\/\\^$|#\s,]/g,'\\$&') + '(?=(?:(?:[^"]*"){2})*[^"]*$)','g');
			re.sep = sep;
		}
		col = col.trim().split(re) || [""];
		col = col.map(x => x.replace(/"/g,'').trim());
		node.goodtmpl = !((col.length === 1) && (col[0] === "")); 
		return col;
	}
	let template = clean(node.template,',');
	let notemplate = template.length === 1 && template[0] === '';
	node.hdrSent = false;
...
//										let p = RED.util.ensureString(RED.util.getMessageProperty(msg,"payload["+s+"]['"+template[t]+"']"));
				...
				try {
					let f = true; // flag to indicate if inside or outside a pair of quotes true = outside.
					let j = 0; // pointer into array of template items
					let k = [""]; // array of data for each of the template items
					let o = {}; // output object to build up
					let a = []; // output array is needed for multiline option
					let first = true; // is this the first line
					let last = false;
					let line = msg.payload;
					let linecount = 0;
					let tmp = "";
					let has_parts = msg.hasOwnProperty("parts");
					let reg = /^[-]?(?!E)(?!0\d)\d*\.?\d*(E-?\+?)?\d+$/i;
					...
					if (node.multi !== "one") {
						...
						let len = a.length;
						for (let i = 0; i < len; i++) {
							let newMessage = RED.util.cloneMessage(msg);
