// from node-red-nodes/hardware/neopixel/colours.js
...
for (let i = 0; i < list.length; i += 2) {
	const name = list[i], value = list[i + 1];
	const r = parseInt(value.slice(1,3), 16);
	const g = parseInt(value.slice(3,5), 16);
	const b = parseInt(value.slice(5), 16);
	colors.set(name, (r << 16) | (g << 8) | b);
}

const p1 = /^\#[a-f0-9]{6}$/;

function getRGB(col, result = {}) {
    col = col.toString().toLowerCase();
    let value = colors.get(col);
    if (undefined !== value) {
